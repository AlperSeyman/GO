...
// get method --> /teachers/{teacher_id}/students
func GetStudentsByTeacherID(w http.ResponseWriter, r *http.Request) {
	...
	students, err := sqlconnect.GetStudentsByTeacher(id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// get method --> /teachers/{teacher_id}/studentCount
func GetStudentCountByTeacherId(w http.ResponseWriter, r *http.Request) {

	idStr := r.PathValue("teascher_id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
		http.Error(w, "Invalid Teacher ID", http.StatusBadRequest)
	}

	studentCount, err := sqlconnect.GetStudentCountByTeacher(id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
